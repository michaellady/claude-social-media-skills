...
- `voice_corpus_freshness` — read from `_shared/voice-corpus/cache.json` via `voice-corpus --print-only`: `newsletter_fetched_at` ← `.fetched_at`, `youtube_transcript_fetched_at` ← `.youtube_fetched_at`, and the two counts from `.posts[].source_type`. **Use the canonical source_type strings** (must match `voice-corpus` main.go + `dashboard/raw.go`): `num_youtube` = count of `source_type` in `{"youtube_live","youtube_transcript"}`; `num_newsletter` = count of `"newsletter"` OR empty/missing (legacy untagged posts default to newsletter). Do **not** query `"youtube"`/`"livestream"` — voice-corpus never emits those, so the count silently reads 0 even when transcripts exist. jq: `num_youtube = [.posts[]|select(.source_type=="youtube_live" or .source_type=="youtube_transcript")]|length`. Nulls/zeros if the cache is absent. (Ties the single pane to the dual-source voice corpus.)
