...
When a dead channel is identified, document the investigation at `audits/<channel>-dead-channel.md` (e.g. `audits/threads-evc-dead-channel.md`) — root cause, evidence, remediation options, decision. `audit-buffer-queue` flags below-threshold channels each weekly run; the audit doc is the deeper one-time investigation. `/flywheel` channel-ROI scoring (yellow_mid / red_dead buckets in `channel_roi[]`) is the surfacing mechanism.

**Enforcement (structural, not just advisory).** Dead-by-engagement channels are skipped at the **transport layer** via `_shared/buffer-post-prep`'s deny-list (`dead-channels.json`, gitignored override `dead-channels.local.json`). When a compose skill routes a post through `buffer-post-prep` for a deny-listed channel, the binary exits `75` and emits `{"skipped": true, "reason": "..."}` instead of post args — so the skill skips it. This complements the prose-level `min_followers_to_promote = 50` guard, which only catches *low-follower* channels, not *zero-engagement* ones. Seeded with Threads `enterprisevibecode` (`6935604f29ea336fd65bacf8`). `audit-buffer-queue` Phase 5b flags any *queued* post targeting a deny-listed channel (i.e. one that bypassed `buffer-post-prep`). See [`_shared/buffer-post-prep/README.md`](_shared/buffer-post-prep/README.md). (Note: the audit doc recommended *reduced cadence*; the snapshots + this enforcement treat it as a hard pause — remove the row from `dead-channels.json` to soften back to reduced cadence.)
