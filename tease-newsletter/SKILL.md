...
- **Invented specifics the source only *entails*.** The adversarial panel's #1 teaser FAIL mode (4-round convergence on "I turned 35… B2B SaaS", 2026-06-19): stating a numeric/duration horizon the article doesn't *literally* write, even one it entails. The source said "wrestling since 8th grade" + "black belt this year" (entails >a decade) and framed goals as "before 36" (~1 year) — but the drafts' "One I trained in for over a decade" and "planning the next ten years" both got FAILed by the strict reviewers (composer-2.5, grok-build) as fabricated specifics while the lenient ones passed (FAIL-OR gate → blocked). **Default to qualitative framing** ("a sport I've trained in for years", "got serious about recently"). A number in a teaser must be lifted faithfully from the source, never inferred.
- **The shortest variant spoils the punchline most easily.** Compressing to Threads length tempts you to *resolve* the tension to land the line — the first short draft said "…it might be the whole point", which stated the article's reframe. Hold the SAME open gap in every length: the short version must withhold exactly what the long one does (e.g. "until a reframe flipped it, and one priority rose above both" — names that a priority exists, never which).
